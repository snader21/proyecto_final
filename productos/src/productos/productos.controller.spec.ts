...
describe('ProductosController', () => {
  ...
  describe('obtenerCategorias', () => {
    it('should return an array of categories', async () => {
      const result: CategoriaEntity[] = [{ id_categoria: '1', nombre: 'Test Category' } as CategoriaEntity];
  ...
  describe('obtenerMarcas', () => {
    it('should return an array of brands', async () => {
      const result: MarcaEntity[] = [{ id_marca: '1', nombre: 'Test Brand' } as MarcaEntity];
  ...
  describe('obtenerUnidadesMedida', () => {
    it('should return an array of measurement units', async () => {
      const result: UnidadMedidaEntity[] = [{ id_unidad_medida: '1', nombre: 'Test Unit' } as UnidadMedidaEntity];
  ...
  describe('obtenerProductosPorPedido', () => {
    it('should return products by order ID', async () => {
      const result: ProductoPorPedidoDto[] = [{ id_producto: '1', nombre: 'Test Product' } as ProductoPorPedidoDto];
      jest.spyOn(service, 'obtenerProductosPorPedido').mockResolvedValue(result);
      expect(await controller.obtenerProductosPorPedido('test-id')).toBe(result);
    });
  });

  describe('obtenerProductos', () => {
    it('should return all products', async () => {
      const result: ProductoEntity[] = [{ id_producto: '1', nombre: 'Test Product' } as ProductoEntity];
  ...
  describe('guardarProducto', () => {
    it('should save a product with files', async () => {
      ...
      expect(await controller.guardarProducto(JSON.stringify(mockProduct), mockFiles)).toBe(result);
    });
  });
